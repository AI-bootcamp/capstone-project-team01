...
def start_game(chessboard = None):
    ...
    st.session_state.white_moves = pd.DataFrame(columns=["Piece", "From", "To", "Eliminated", "Castling Type", "Rook From", "Rook To"])
    st.session_state.black_moves = pd.DataFrame(columns=["Piece", "From", "To", "Eliminated", "Castling Type", "Rook From", "Rook To"])
...
def detect_move(previous_board_status, new_board_status, chessboard, board=None):
    move = {}
    rook_move = {}
    ...
    if 'start' in move and 'end' in move:
        ...
        # Kingside Castling Detection
        if piece in ['wk', 'bk'] and abs(start_col - end_col) == 2:
            if end_col > start_col:  # King moved to g1 or g8
                rook_col = 7  # h1 or h8
                rook_target_col = 5  # f1 or f8
                rook_move = {'start': (start_row, rook_col), 'end': (start_row, rook_target_col)}
                move['castling'] = 'O-O'
                move['rook_start'] = f"{chr(97 + rook_col)}{8 - start_row}"
                move['rook_end'] = f"{chr(97 + rook_target_col)}{8 - start_row}"
            else:  # Queenside Castling (King moved to c1 or c8)
                rook_col = 0  # a1 or a8
                rook_target_col = 3  # d1 or d8
                rook_move = {'start': (start_row, rook_col), 'end': (start_row, rook_target_col)}
                move['castling'] = 'O-O-O'
                move['rook_start'] = f"{chr(97 + rook_col)}{8 - start_row}"
                move['rook_end'] = f"{chr(97 + rook_target_col)}{8 - start_row}"

    # Update Move History for Castling
    if rook_move:
        move['rook'] = rook_move
...
def export_to_pdf(white_moves, black_moves):
    ...
    for index, row in white_moves.iterrows():
        c.drawString(100, y, f"{row['Piece']} from {row['From']} to {row['To']} (Eliminated: {row['Eliminated']}) | Castling: {row['Castling Type']} | Rook: {row['Rook From']} -> {row['Rook To']}")
        y -= 20
